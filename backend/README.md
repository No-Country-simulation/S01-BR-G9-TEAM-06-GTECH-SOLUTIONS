...
O perfil `local` conecta exclusivamente ao MySQL publicado em `localhost`,
permite a recuperação da chave pública exigida pelo `caching_sha2_password` do
MySQL 8.4 e desabilita o atributo `Secure` do cookie para uso por HTTP. Esse
perfil não deve ser usado em deploys ou para acessar bancos remotos.
...
|---|---:|---|
| `SPRING_PROFILES_ACTIVE` | vazio | Use `local` somente com o MySQL do Compose em `localhost`; não configure no deploy |
